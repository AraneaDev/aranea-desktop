...
// --- stack split
const u = list => list.map(x => ({ urgency: x }))
let split = inbox.stackSplit(u([1, 1]), 3)
assert(split.visible.join() === '0,1' && split.overflow === 0, 'small stack shows everything')
split = inbox.stackSplit(u([1, 1, 1, 1, 1]), 3)
assert(split.visible.join() === '0,1,2' && split.overflow === 2, 'newest three shown, rest overflow')
split = inbox.stackSplit(u([1, 1, 1, 2]), 3)
assert(split.visible.join() === '0,1,3' && split.overflow === 1, 'critical takes a visible slot first')
split = inbox.stackSplit(u([2, 2, 2, 2]), 3)
assert(split.visible.join() === '0,1,2' && split.overflow === 1, 'newest criticals win when criticals overflow')
...
assert(inbox.tooltipText(1, false, false) === '1 notification · DND off', 'singular tooltip')
assert(inbox.tooltipText(4, true, false) === '4 notifications · DND on', 'plural tooltip with DND')
assert(inbox.tooltipText(0, false, true) === '0 notifications · Quiet hours', 'quiet-hours tooltip')
