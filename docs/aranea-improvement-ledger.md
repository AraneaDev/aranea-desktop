...
| 4. Local install and rollback | complete | pending commit | Installer accepts `--source PATH|URL`, rejects missing local paths, preserves remote default behavior, and prints a previous-theme restore command on failure | Full failure injection against `omarchy theme install` remains a future hardening test |
...
| 2026-09-22 | Task 4 | `bash tests/install.test.sh` and `shellcheck -x scripts/install.sh tests/install.test.sh`: passed; local-source dry-run and missing-source rejection covered |
