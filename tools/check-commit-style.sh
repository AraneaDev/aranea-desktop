...
#
# release-please reads the subject that lands on master to build the changelog
# and choose the next version, so a subject it cannot parse is not a style
# slip: the change silently misses the changelog, and a fix that should have
# moved the version leaves it where it was.
#
# This repository squash merges, so the subject release-please eventually
# reads is the pull request title, not any of the commits inside it. CI
...
# with nothing in it, which is a different thing and not this script's to
# judge. Both look identical through "${1:-}", so the count is what separates
# them.
[ "$#" -gt 0 ] || usage
...
case "$1" in
  --file)
    ...
    # edited carries git's comments above it, and a template can push the
    # real subject further down still.
...
# and release-please's own release subject is exempt as belt and braces.
# None of them reach master in a form this rule should judge.
case "$subject" in
  'Merge '* | 'Revert "'* | 'fixup!'* | 'squash!'* | 'amend!'* | 'chore(master): release '*) exit 0 ;;
esac
...
# check exists to prevent -- cannot happen. jq is installed by the job that
# calls this in CI; the commit-msg hook may run on a machine without it, so
# that one case falls back to the conventional set, which is a superset of
# any sane config.
...
fail() {
  ...
  printf '    feat: add the icons integration\n' >&2
  printf '    fix: stop the folder icon from clashing with the theme\n' >&2
  printf '    docs(readme): document the icons integration\n' >&2
  printf '    feat!: a break, or use a BREAKING CHANGE: trailer\n\n' >&2
  printf '  Types: %s\n\n' "$(printf '%s' "$types" | tr '|' ' ')" >&2
  exit 1
}

# A type, an optional scope, an optional ! for a break, then ": " and
# something to say. Anchored at both ends so a subject that merely mentions a
# type somewhere does not pass.
if ! printf '%s' "$subject" |
